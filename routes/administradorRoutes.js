...
router.put('/log/administrador/editar/:id', async (req, res) => {
  const { id } = req.params;
...
//editar contrasena desde el administrador a un administrador (en caso de que la olvide)
...
//editar contrasena de una delgacion desde el administrador, esto cuando una delegacion olvido su contra, el admin le cambia para que pueda acceder
router.put('/log/administrador/editar_contra_delegacion/:id', async (req, res) => {
  const { id } = req.params;
  const { nueva_contrasena_delegacion } = req.body;
  try {
      // Validar campo requerido
      if (!nueva_contrasena_delegacion) {
          return res.status(400).json({ message: 'Campo obligatorio.' });
      }
      // Encriptar contraseña
      const hashedPassword = bcrypt.hashSync(nueva_contrasena_delegacion, 10);
      ...
      if (error) {
          ...
          return res.status(500).json({ message: 'Error al actualizar la contraseña de la delegación' });
      }
      // Verificar si no se actualizó nada
      if (!data || data.length === 0) {
          return res.status(404).json({ message: 'Delegación no encontrada' });
      }
      // Respuesta exitosa
      res.json({ message: 'Se actualizó correctamente la contraseña de la delegación' });
  } catch (err) {
      res.status(500).json({ error: err.message || err });
  }
});
